...
Kilnr is CI infrastructure and executes repository-controlled code.

A Kilnr installation should therefore be treated as privileged infrastructure even though individual build containers are deliberately restricted.
...
- the affected Kilnr version or commit;
...
Kilnr deliberately separates several trust domains:

- `git` owns bare repositories and receives SSH pushes;
- `kilnr` controls CI state and orchestration;
- build containers receive repository code but not the Docker socket;
- `/etc/kilnr/secrets` is never mounted into build containers;
- Kilnr Web is read-only and runs separately from the controller;
- CI containers use resource limits and a dedicated network;
- private/LAN networks are blocked from the default CI network.

The Docker daemon itself remains privileged infrastructure. The Kilnr controller's Docker access must therefore be treated accordingly.
...
- tokens;
- passwords;
- `/etc/kilnr/secrets` contents;
- private runtime configuration copied from a Kilnr server.
